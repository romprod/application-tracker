...
export const outlookConnectionReconciliationMigration: Migration = {
  name: "outlook_connection_reconciliation",
  version: 32,
  sql: `
    ALTER TABLE outlook_graph_connections
      ADD COLUMN last_reconciled_at TEXT
        CHECK (
          ...
          length(trim(last_reconciled_at)) > 0
        );
    ...
    ALTER TABLE mcp_audit_events RENAME TO mcp_audit_events_version_thirty_one;
    ...
    SELECT
      id, workspace_id, actor_user_id, transport, action, target_type, result,
      occurred_at
    FROM mcp_audit_events_version_thirty_one;

    DROP TABLE mcp_audit_events_version_thirty_one;
