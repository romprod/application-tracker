...
export const applicationAttentionQueueMigration: Migration = {
  name: "application_attention_queue",
  version: 38,
  sql: `
    CREATE INDEX application_attention_status_updated
      ON applications (
        workspace_id, status_reference_id, updated_at DESC, id DESC
      ) WHERE deleted_at IS NULL;
    CREATE INDEX application_attention_next_action
      ON applications (
        workspace_id, next_action_due, updated_at DESC, id DESC
      ) WHERE deleted_at IS NULL;
    CREATE INDEX application_attention_applied
      ON applications (workspace_id, applied_on, updated_at DESC, id DESC)
      WHERE deleted_at IS NULL;
    CREATE INDEX application_attention_company_role
      ON applications (
        workspace_id, lower(trim(company_name)), lower(trim(role_title)), id
      ) WHERE deleted_at IS NULL;
    CREATE INDEX application_email_evidence_by_application_type
      ON application_email_evidence (
        workspace_id, application_id, evidence_type, received_at DESC, id
      );
    ...
    ALTER TABLE mcp_audit_events RENAME TO mcp_audit_events_version_thirty_seven;
    ...
    CREATE TABLE mcp_audit_events (
      ...
      action TEXT NOT NULL CHECK (action IN (
        ...
        'list_applications', 'get_application', 'list_application_events',
        'list_unlinked_applications', 'get_application_data_quality',
        'audit_duplicate_applications', 'find_duplicate_applications',
        'merge_applications', 'match_job_application_email',
    ...
    FROM mcp_audit_events_version_thirty_seven;
    DROP TABLE mcp_audit_events_version_thirty_seven;
